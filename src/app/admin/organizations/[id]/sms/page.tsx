...
export default async function AdminOrganizationSmsPage({
  ...
  const [metrics, testSteps] = await Promise.all([
    loadSmsActivityMetrics(id, safeSender),
    Promise.resolve(deriveSmsTestDeliveryResults(safeSender))
  ]);

  return (
    <section className="mx-auto grid max-w-5xl gap-5 pb-8">
      <SmsPageHeader organizationName={organization.name} />
