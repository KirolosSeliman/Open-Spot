-- Additive hotfix: replace invalid schema-qualified SQL conditional
-- expressions in RPC definitions that may already be deployed.
--
-- COALESCE and NULLIF are SQL conditional expressions, not callable
-- pg_catalog functions. Keep valid pg_catalog function references for normal
-- functions such as btrim, lower, length, now, and jsonb_build_object.
