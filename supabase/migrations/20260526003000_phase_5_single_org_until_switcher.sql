-- Phase 5 blocker hardening: enforce a single organization membership per
-- user until the product has an explicit organization switcher.
--
-- This is intentionally temporary. A future organization-switcher migration
-- must drop organization_members_single_org_per_user_idx before allowing a
-- user to belong to more than one organization.
...
begin
  if exists (
    ...
    raise exception 'Cannot enforce single-organization mode because duplicate user memberships already exist. Resolve duplicates manually before applying this migration.';
  end if;
end;
$$;

create unique index if not exists organization_members_single_org_per_user_idx
